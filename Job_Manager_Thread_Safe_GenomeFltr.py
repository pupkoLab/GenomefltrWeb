...
class Job_Manager_Thread_Safe_GenomeFltr:
    ...
    def __init__(self, max_number_of_process: int, upload_root_path: str, input_file_names: list, func2update_db_donwload, func2update_html_kraken, func2update_html_output, func2update_html_postprocess):
        """
        ...
        func2update_html_output: function
            What function should be called once the state is cahnged in the output processes
        ...
        self.__func2update_html_output = func2update_html_output
        ...
        function2call_processes_changes_state = {
            ...
            kc.OUTPUT_PROCESSOR_JOB_PREFIX: self.__func2update_html_output,
        ...
        function2append_process = {
            ...
            kc.OUTPUT_PROCESSOR_JOB_PREFIX: self.__output_process,
        ...
        paths2verify_process_ends = {
            ...
            kc.KRAKEN_JOB_PREFIX: [lambda process_id: os.path.join(os.path.join(upload_root_path, process_id), kc.KRAKEN_RESULTS_FILE_NAME)],
            kc.OUTPUT_PROCESSOR_JOB_PREFIX: [lambda process_id: os.path.join(os.path.join(upload_root_path, process_id), sc.K_MER_COUNTER_MATRIX_FILE_NAME)],
    ...
    def __download_process(self, process_folder_path: str, email_address, job_name, species2download, list_accession_numbers):
        ...
        ** Specific to this webserver
        ...
        logger.info(f'pbs_id = {pbs_id} type = {type(pbs_id)}')
        if isinstance(pbs_id, str):
            try:
                pbs_id_dict = ast.literal_eval(pbs_id)
                if 'job_id' in pbs_id_dict:
                    logger.info(f'returned pbs_id_dict = {pbs_id_dict["job_id"]}')
                    return pbs_id_dict['job_id']
    ...
    def __kraken_process(self, process_folder_path: str, email_address, job_name, db_type):
        ...
        ** Specific to this webserver
        ...
        pbs_id = self.__search_engine.kraken_search(files2fltr, None, db_type)
        logger.info(f'pbs_id = {pbs_id} type = {type(pbs_id)}')
        if isinstance(pbs_id, str):
            try:
                pbs_id_dict = ast.literal_eval(pbs_id)
    ...
    def __output_process(self, process_folder_path: str):
        """
        Creates a output processor
        ** Specific to this webserver
        ...
        logger.info(f'process_folder_path = {process_folder_path}')
        files2fltr = self.__get_files_in_folder(process_folder_path)
        pbs_id = self.__search_engine.output_processor(files2fltr)
        logger.info(f'pbs_id = {pbs_id} type = {type(pbs_id)}')
        if isinstance(pbs_id, str):
            try:
                pbs_id_dict = ast.literal_eval(pbs_id)
    ...
    def get_output_job_state(self, process_id):
        """
        Get kraken job state
        ** Specific to this webserver
        ...
        state: State (Enum)
            The state of the process
        """
        return self.__get_state(process_id, kc.OUTPUT_PROCESSOR_JOB_PREFIX)
    ...
    def add_output_process(self, process_id: str):
        """
        adds a kraken process
        ** Specific to this webserver
        ...
        self.__job_manager.add_process(process_id, kc.OUTPUT_PROCESSOR_JOB_PREFIX)
