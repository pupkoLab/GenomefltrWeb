...
from KrakenConsts import KRAKEN_CUSTOM_DB_NAME_PREFIX, KRAKEN_JOB_PREFIX, OUTPUT_PROCESSOR_JOB_PREFIX
from SharedConsts import K_MER_COUNTER_MATRIX_FILE_NAME, \
    FINAL_OUTPUT_FILE_NAME, FINAL_OUTPUT_ZIPPED_BOTH_FILES, KRAKEN_SUMMARY_RESULTS_FOR_UI_FILE_NAME, EMAIL_CONSTS, \
    UI_CONSTS, CUSTOM_DB_NAME, State, POSTPROCESS_JOB_PREFIX, GENOME_DOWNLOAD_SUMMARY_RESULTS_FILE_NAME, \
    FINAL_OUTPUT_FILE_CONTAMINATED_NAME, FINAL_OUTPUT_ZIPPED_BOTH_FILES_NEW_CONTAMINATED, \
    SPECIAL_MARTHA_JOB_NAME_STRING, SPECIAL_MARTHA_DB_NAME
...
class Job_Manager_API:
    ...
    def __init__(self, max_number_of_process: int, upload_root_path: str, input_file_names: list, func2update_html):
        ...
        self.__j_manager = Job_Manager_Thread_Safe_GenomeFltr(max_number_of_process, upload_root_path, input_file_names, self.__update_download_process,
                                                             self.__process_kraken_changed, self.__process_state_changed, self.__process_state_changed)
        self.input_validator = InputValidator() # creates the input_validator
        self.__func2update_html = func2update_html
        self.EXAMPLE_FOLDER_PATH = r'/genomefltr/example_process_results/'
    ...
    def __process_kraken_changed(self, process_id, state, email_address, job_name, job_prefix):
        ...
        if state == State.Finished:
            logger.info(f'process_id = {process_id} state = {state}')
            # once the kraken finished, start the output processor job
            self.__build_and_send_mail(process_id, f'Kraken job finished', f'\n\nemail adress of the user: {email_address}' + f'\n\nprocess_id: {process_id}', 'edodotan@mail.tau.ac.il')
            self.__j_manager.add_output_process(process_id)
        elif state == State.Crashed:
            logger.info(f'process_id = {process_id} state = {state}')
            self.__build_and_send_mail(process_id, EMAIL_CONSTS.create_title(state, job_name), EMAIL_CONSTS.CONTENT_CRASHED_DOWNLOAD_PROCESS.format(process_id=process_id), email_address)
            self.__build_and_send_mail(process_id, EMAIL_CONSTS.create_title(state, job_name), EMAIL_CONSTS.CONTENT_CRASHED_DOWNLOAD_PROCESS.format(process_id=process_id) + f'\n\nemail adress of the user: {email_address}' + f'\n\nprocess_id: {process_id}', 'edodotan@mail.tau.ac.il')
    ...
    def __process_state_changed(self, process_id, state, email_address, job_name, job_prefix):
        ...
        if email_address != None:
            ...
            if state == State.Finished:
                if job_prefix == OUTPUT_PROCESSOR_JOB_PREFIX:
    ...
    def add_kraken_process(self, process_id: str, email_address: str, job_name: str, db_type: str, species2download: list, list_accession_numbers: list):
        ...
        if is_valid_email:
            if job_name != "" and job_name.find(SPECIAL_MARTHA_JOB_NAME_STRING) != -1:
                db_type = SPECIAL_MARTHA_DB_NAME
    ...
    def get_output_processor_job_state(self, process_id):
        """Given process_id returns the output processor job state
        ...
        -------
        """
        return self.__j_manager.get_output_job_state(process_id)
