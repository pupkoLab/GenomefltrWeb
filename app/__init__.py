from flask import Flask, flash, request, redirect, url_for, render_template, Response, jsonify, send_file, send_from_directory
...
from KrakenConsts import KRAKEN_DB_NAMES
...
UPLOAD_FOLDERS_ROOT_PATH = '/genomefltr/user_results/' # path to folder to save results
...
def process_state(process_id):
    ...
    job_state_output_processor = manager.get_output_processor_job_state(process_id)
    logger.debug(f'app.process_state: job_state_kraken = {job_state_kraken} job_state_download = {job_state_download} job_state_output_processor = {job_state_output_processor}')
    if job_state_kraken == None and job_state_download == None and job_state_output_processor == None:
        return redirect(url_for('error', error_type=UI_CONSTS.UI_Errors.UNKNOWN_PROCESS_ID.name))
    
    if job_state_kraken == State.Crashed or job_state_output_processor == State.Crashed:
    ...
    if job_state_output_processor != State.Finished:
        # here we decide what GIF will be displayed to the user
        # if job_state_output_processor == None than the output processor havent start running
        if job_state_output_processor == None:
            kwargs = {
        ...
        # output processor is running
        else:
            kwargs = {
                "process_id": process_id,
                "text": UI_CONSTS.states_text_dict[job_state_output_processor],
                "gif": UI_CONSTS.states_gifs_dict[job_state_output_processor],
                "message_to_user": UI_CONSTS.PROCESS_INFO_KR,
                "update_text": UI_CONSTS.TEXT_TO_RELOAD_HTML,
                "update_interval_sec": UI_CONSTS.FETCH_UPDATE_INTERVAL_HTML_SEC
            }
...
def preview():
    return send_file(f'static/images/graphical_abstract_overview_page_preview.png', mimetype='image/png')
...
def monitor():
    ...
    for process_id in list(processes_state_dict):
        running_jobs_list[process_id] = {'KR':[], 'OP':[], 'PP':[]}
        finished_jobs_list[process_id] = {'KR':[], 'OP':[], 'PP':[]}
        for prefix in job_prefixes:
            state = processes_state_dict[process_id].get_job_state(prefix)
            if state == State.Finished:
                finished_jobs_list[process_id][prefix].append (f'{process_id}, {state}')
            elif state == State.Crashed:
                count_crashed += 1
            elif state == State.Init:
                count_init += 1
            else:
                running_jobs_list[process_id][prefix].append (f'{process_id}, {state}')
            
    for process_id in list(waiting_list):
        for prefix in job_prefixes:
            waiting_jobs_list[prefix].append (f'{process_id[0]}, State.Waiting')
    
    processes_state_dict_json = {
    k: vars(v)
    for k, v in processes_state_dict.items()
    }

    return render_template('monitor.html', processes_list=processes_state_dict.keys(), processes_state_dict = processes_state_dict, job_prefixes = job_prefixes, count_finished = count_finished, count_crashed = count_crashed, count_init = count_init, running_jobs_list = running_jobs_list, waiting_jobs_list = str(waiting_list), finished_jobs_list = finished_jobs_list)
    '''
    running_jobs_list = [1,2,3] 
    waiting_jobs_list = [3,4,5]
    
    return render_template('monitor.html', count_finished = 0, count_crashed = 0, count_init = 0, running_jobs_list = running_jobs_list, waiting_jobs_list = waiting_jobs_list)
    '''