...
from KrakenHandlers.DbUpdater import DbUpdater
...
class Job_Manager_Thread_Safe:
    ...
    def __init__(self, max_number_of_process: int, upload_root_path: str, function2call_processes_changes_state: dict,
        ...
        self.__scheduler.add_job(self.__db_updator.run_db_update, 'interval', minutes= 60 * 24 * 3) # three days
        #self.__scheduler.add_job(self.__db_updator.run_db_update, 'interval', minutes= 1) 
    ...
    def add_process(self, process_id: str, job_prefix, *args):
        ...
        if running_processes < self.__max_number_of_process:
            ...
            try:
                ...
                logger.debug(f'process_id = {process_id} __processes_state_dict[process_id] = {self.__processes_state_dict[process_id]} state = {self.__processes_state_dict[process_id].get_job_state(job_prefix)}')
            except Exception as e:
                logger.error(e)
    ...
    def get_job_state(self, process_id: str, job_prefix: str):
        ...
        if state2return == None:
            ...
            self.__mutex_processes_waiting_queue.release()
        self.__mutex_processes_state_dict.release()
        logger.info(f'state2return = {state2return} job_prefix = {job_prefix} process_id = {process_id}')
        logger.info(f'self.__processes_state_dict.keys() = {self.__processes_state_dict.keys()}')
        return state2return
    ...
    def get_processes_state_dict(self):
        return self.__processes_state_dict
